    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          <h3 style={{ margin: 0, fontSize: 14, color: '#e0e2ed' }}>
            待审批订单
            {pending.length > 0 && (
              <span style={{
          ...
          <button
            ...
            onClick={loadData}
            style={{
              background: 'none', border: '1px solid rgba(66, 71, 84, 0.15)',
        ...
        <div className="table-shell data-table-shell">
            <table className="data-table" style={{ tableLayout: 'fixed', width: '100%' }}>
              <thead>
                <tr>
                  <th className="s-center" style={{ width: '3%' }}>
                  ...
                  <th className="s-right" style={{ width: '7%' }}>股数</th>
                  <th className="s-right" style={{ width: '9%' }}>金额</th>
                  <th style={{ width: '10%' }}>策略</th>
                  <th style={{ width: '12%' }}>信号</th>
                  <th className="s-right" style={{ width: '7%' }}>风控分</th>
                  <th style={{ width: '8%' }}>状态</th>
                  <th className="s-center" style={{ width: '11%' }}>操作</th>
                </tr>
              </thead>
              <tbody>
                {pending.length === 0 && (
                  <tr>
                    <td colSpan={12} className="s-center s-text-muted" style={{ padding: '32px 0' }}>
                      当前无待审批订单
                    </td>
                  </tr>
                )}
                {pending.map(o => (
                  <tr key={o.id}>
                    ...
                    <td>
                      <span style={{
                        fontWeight: 600,
                        color: o.direction === 'BUY' ? '#ff5451' : '#22C55E',
                      }}>
                        {o.direction === 'BUY' ? '买入' : '卖出'}
                      </span>
                    </td>
                    <td className="numeric s-right">{o.order_shares?.toLocaleString()}</td>
                    <td className="numeric s-right">{formatWan(o.order_amount)}</td>
                    ...
                    <td className="s-center" style={{ whiteSpace: 'nowrap' }}>
                      <button
                        type="button"
                        disabled={busy}
                        onClick={() => handleApprove([o.id])}
                        style={{
                          background: 'none', border: '1px solid #ff5451',
                          borderRadius: 3, padding: '2px 8px', fontSize: 11,
                          color: '#ff5451', cursor: 'pointer', marginRight: 4,
                        }}
                      >批准</button>
                      <button
                        type="button"
        ...
        {selectedIds.size > 0 && (
          <div style={{
            ...
            background: '#1c2027', borderRadius: 5,
            border: '1px solid rgba(66, 71, 84, 0.15)',
          }}>
            <span style={{ fontSize: 13, color: '#c2c6d6' }}>
            ...
            <button
              ...
              style={{
                background: 'rgba(34,197,94,0.12)', border: '1px solid #ff5451',
                borderRadius: 5, padding: '5px 16px', fontSize: 13,
                fontWeight: 600, color: '#ff5451', cursor: 'pointer',
              }}
            ...
            <button
              ...
              style={{
                background: 'rgba(220,38,38,0.08)', border: '1px solid #22C55E',
                borderRadius: 5, padding: '5px 16px', fontSize: 13,
                fontWeight: 600, color: '#22C55E', cursor: 'pointer',
              }}
      ...
      {processed.length > 0 && (
        <div>
          <h3 style={{ margin: '0 0 12px', fontSize: 14, color: '#c2c6d6' }}>
            最近已处理
          </h3>
                ...
                {processed.map(o => (
                  <tr key={o.id}>
                    ...
                    <td>
                      ...
                      {o.stock_name && <span style={{ marginLeft: 6, fontSize: 12, color: '#c2c6d6' }}>{o.stock_name}</span>}
                    </td>
                    <td>
                      <span style={{ fontWeight: 600, color: o.direction === 'BUY' ? '#ff5451' : '#22C55E' }}>
                        {o.direction === 'BUY' ? '买入' : '卖出'}
                      </span>
                    </td>
                    <td className="numeric s-right">{formatWan(o.order_amount)}</td>
                    ...
                    <td style={{ fontSize: 12, color: '#8c909f' }}>{o.approved_by || '--'}</td>
                    <td className="numeric" style={{ fontSize: 12, color: '#8c909f' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="stat-card" style={{ padding: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 13, color: '#c2c6d6', marginBottom: 6 }}>当前审批模式</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: mode === 'AUTO' ? '#3B82F6' : '#F59E0B' }}>
              {mode === 'AUTO' ? '自动模式' : '审批模式'}
            </div>
            <div style={{ fontSize: 12, color: '#8c909f', marginTop: 4 }}>
              {mode === 'AUTO'
                ? '所有新订单自动放行执行，无需人工审批'
                : '新订单按分级规则审批，部分需人工确认'}
            </div>
      ...
      {confirmAction && (
        <div style={{
          ...
          <div style={{
            background: '#181c23', borderRadius: 8, padding: 24,
            maxWidth: 420, width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.55)',
          }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 15, color: '#e0e2ed' }}>
              确认切换
            </h3>
            <p style={{ fontSize: 13, color: '#c2c6d6', lineHeight: 1.6, margin: '0 0 20px' }}>
      ...
      <div className="stat-card" style={{ padding: 20 }}>
        <h3 style={{ margin: '0 0 14px', fontSize: 14, color: '#e0e2ed' }}>
          分级审批规则
        </h3>
        <div style={{ fontSize: 13, color: '#c2c6d6', display: 'flex', flexDirection: 'column', gap: 10 }}>
  ...
  return (
    <div className="table-shell data-table-shell">
      <table className="data-table">
        <thead>
          <tr>
            <th style={{ minWidth: 80 }}>股票代码</th>
            <th style={{ minWidth: 90 }}>股票名称</th>
            <th>方向</th>
            <th className="s-right">股数</th>
            <th className="s-right">入场价</th>
            <th className="s-right">最新价</th>
            <th className="s-right">浮盈%</th>
            <th className="s-right">持仓天数</th>
            <th style={{ minWidth: 80 }}>策略</th>
            <th className="s-right">风控评分</th>
            <th className="s-right">仓位系数</th>
            <th style={{ minWidth: 80 }}>执行状态</th>
          </tr>
        </thead>
        <tbody>
          {rows.length === 0 && (
          ...
          {rows.map((row) => {
            ...
            return (
              <tr key={row.id}>
                ...
                <td>{row.name}</td>
                <td>
                  <span style={{ fontWeight: 600, color: '#ff5451' }}>持仓</span>
                </td>
                <td className="numeric s-right">{row.shares.toLocaleString()}</td>
                <td className="numeric s-right">{formatNumber(row.entryPrice)}</td>
                <td className="numeric s-right">{formatNumber(latestPrice)}</td>
                <td className="numeric" style={{
                  textAlign: 'right', fontWeight: 600,
                  color: pnlPct > 0 ? '#ff5451' : pnlPct < 0 ? '#22C55E' : '#c2c6d6',
                }}>
                ...
                <td className="numeric s-right">{row.riskLevelLabel}</td>
                <td className="numeric s-right">{row.positionCapText}</td>
                <td>
                  <span style={{
  ...
  return (
    <div className="table-shell data-table-shell">
      <table className="data-table">
        <thead>
          <tr>
            <th style={{ minWidth: 85 }}>成交日期</th>
            <th style={{ minWidth: 80 }}>股票代码</th>
            <th style={{ minWidth: 90 }}>股票名称</th>
            <th>方向</th>
            <th className="s-right">成交价</th>
            <th className="s-right">成交股数</th>
            <th className="s-right">成交金额</th>
            <th style={{ minWidth: 80 }}>策略</th>
            <th style={{ minWidth: 100 }}>信号类型</th>
        ...
        <tbody>
          {rows.length === 0 && (
            <tr>
              <td colSpan={10} className="s-center s-text-muted" style={{ padding: '32px 0' }}>
                暂无成交数据
              </td>
            </tr>
          )}
            ...
            return (
              <tr key={row.id}>
                ...
                <td>
                  <span style={{
                    fontWeight: 600,
                    color: row.side === 'buy' ? '#ff5451' : '#22C55E',
                  }}>
                    {row.side === 'buy' ? '买入' : '卖出'}
                  </span>
                </td>
                <td className="numeric s-right">{formatNumber(row.fillPrice)}</td>
                <td className="numeric s-right">{row.fillQty.toLocaleString()}</td>
                <td className="numeric s-right">{formatWan(fillAmount)}</td>
                <td style={{ fontSize: 12 }}>{strategyDisplay}</td>
