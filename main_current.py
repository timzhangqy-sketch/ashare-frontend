...
def get_watchlist_active(strategy: str = None):
    ...
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    base_sql = """
        SELECT va.id, va.ts_code, va.name, va.strategy, va.entry_date::text,
               va.entry_price, va.entry_score, va.entry_rank, va.pool_day,
               va.latest_close, va.gain_since_entry, va.max_gain,
               va.drawdown_from_peak, va.vr_today, va.turnover_rate,
               va.ma5, va.ma10, va.ma20, va.above_ma20_days,
               va.buy_signal, va.sell_signal, va.tags,
               -- daily_price: amount
               ROUND((dp.amount / 10000.0)::numeric, 2) AS amount_yi,
               -- VOL_SURGE extras
               vsp.avg_vr3,
               ROUND((vsp.ret5)::numeric, 4) AS ret5_pct,
               ROUND((vsp.ret20)::numeric, 4) AS ret20_pct,
               -- RETOC2 extras
               COALESCE(rt.total_bars_10, rt2w.total_bars_10) AS retoc_cnt,
               COALESCE(rt.ret10, rt2w.ret10) AS ret10,
               rt.bars_today,
               -- T2UP9 extras
               t2.ret_t2,
               t2.ret_2d AS ret_2d_cum
        FROM public.v_watchlist_active va
        LEFT JOIN LATERAL (
            SELECT amount FROM ashare_daily_price
            WHERE ts_code = va.ts_code ORDER BY trade_date DESC LIMIT 1
        ) dp ON true
        LEFT JOIN LATERAL (
            SELECT avg_vr3, ret5, ret20 FROM ashare_vol_surge_pool
            WHERE ts_code = va.ts_code AND va.strategy = 'VOL_SURGE'
            ORDER BY trade_date DESC LIMIT 1
        ) vsp ON true
        LEFT JOIN LATERAL (
            SELECT total_bars_10, ret10, cnt_bars AS bars_today, close AS retoc_close FROM ashare_retoc2_v3_trigger
            WHERE ts_code = va.ts_code AND va.strategy = 'RETOC2'
            ORDER BY trade_date DESC LIMIT 1
        ) rt ON true
        LEFT JOIN LATERAL (
            SELECT total_bars_10, ret10 FROM ashare_retoc2_v3_watch
            WHERE ts_code = va.ts_code AND va.strategy = 'RETOC2'
            ORDER BY trade_date DESC LIMIT 1
        ) rt2w ON true
        LEFT JOIN LATERAL (
            SELECT ret_t2, ret_2d FROM ashare_pattern_t2up9_2dup_lt5_candidates
            WHERE ts_code = va.ts_code AND va.strategy = 'PATTERN_T2UP9'
            ORDER BY trade_date DESC LIMIT 1
        ) t2 ON true
    """
    if strategy:
        cur.execute(base_sql + " WHERE va.strategy = %s", (strategy.upper(),))
    else:
...
def get_watchlist_stats():
    ...
    cur.execute("""
        SELECT
            ...
            count(*) FILTER (WHERE status = 'active' AND strategy = 'VOL_SURGE') AS active_vol_surge,
            ...
            count(*) FILTER (WHERE status = 'active' AND strategy = 'WEAK_BUY') AS active_weak_buy,
    ...
    try:
        ...
        try:
            ...
            if pr:
                ...
                # turnover_rate
                cur.execute("""
                    SELECT turnover_rate, pe, pb, total_mv, trade_date FROM ashare_daily_basic
                    WHERE ts_code = %s AND trade_date <= %s
                    ORDER BY trade_date DESC LIMIT 1
                """, (ts_code, eff_date_str))
                db_row = cur.fetchone()
                ...
                pe_ttm = _dec(db_row["pe"]) if db_row else None
                pb = _dec(db_row["pb"]) if db_row else None
                total_mv_wan = _dec(db_row["total_mv"]) if db_row else None

                # prev_close — use actual price date, not eff_date
                actual_price_date = pr["trade_date"]
                prev_td = _prev_trade_date(cur, actual_price_date)
                prev_close = None
                if prev_td:
                    cur.execute("""
                        SELECT close FROM ashare_daily_price
                ...
                cur.execute("""
                    ...
                    SELECT
                        ROUND(AVG(CASE WHEN rn <= 5 THEN close END)::numeric, 2) AS ma5,
                        ROUND(AVG(CASE WHEN rn <= 10 THEN close END)::numeric, 2) AS ma10,
                        ROUND(AVG(CASE WHEN rn <= 20 THEN close END)::numeric, 2) AS ma20
                    FROM recent
                """, (ts_code, eff_date_str))
                ma_row = cur.fetchone()
                ma5 = _dec(ma_row["ma5"]) if ma_row else None
                ma10 = _dec(ma_row["ma10"]) if ma_row else None
                ma20 = _dec(ma_row["ma20"]) if ma_row else None

                # VR from watchlist
                vr = None
                try:
                    cur.execute("""
                        SELECT vr_today FROM ashare_watchlist
                        WHERE ts_code = %s AND status = 'active'
                        ORDER BY entry_date DESC LIMIT 1
                    """, (ts_code,))
                    vr_row = cur.fetchone()
                    if vr_row and vr_row.get("vr_today"):
                        vr = _dec(vr_row["vr_today"])
                except Exception:
                    pass
                ...
                quote = {
                    ...
                    "pe_ttm": pe_ttm,
                    "pb": pb,
                    "total_mv_yi": round(total_mv_wan / 10000, 2) if total_mv_wan else None,
                }
            else:
                quote = None
        except Exception as e:
...
def get_pipeline_runs(trade_date: str = None):
    ...
    try:
        ...
        if not eff:
            return []
        cur.execute("""
            SELECT trade_date, step, status, started_at, ended_at, duration_ms, rowcount, message
...
def get_data_coverage(trade_date: str = None):
    ...
    try:
        ...
        if trade_date:
            ...
            cur.execute("SELECT MAX(trade_date) AS td FROM ashare_daily_price")
            row = cur.fetchone()
            eff = row["td"] if row and row["td"] else _resolve_trade_date(cur)
        eff_str = str(eff) if eff else None

        tables = [
            ("ashare_daily_price", "trade_date"),
            ("ashare_daily_basic", "trade_date"),
            ("ashare_adj_factor", "trade_date"),
            ("ashare_index_daily_price", "trade_date"),
            ("ashare_intraday_5m", "trade_date"),
            ("ashare_fin_income", "ann_date"),
            ("ashare_fin_balance", "ann_date"),
            ("ashare_fin_cashflow", "ann_date"),
            ("ashare_audit_opinion", "ann_date"),
            ("ashare_pledge_stat", "end_date"),
            ("ashare_risk_score", "trade_date"),
            ("ashare_watchlist", "entry_date"),
            ("ashare_vol_surge_pool", "trade_date"),
        ]
        result = []
        for tbl, date_col in tables:
            try:
                cur.execute("""
                    SELECT MAX(""" + date_col + """)::text AS latest_date,
...
def get_sim_checks(trade_date: str = None):
    """Return execution constraint checks: risk gate + position cap + reject reasons."""
    conn = get_db()
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        # resolve trade_date
        if not trade_date:
            cur.execute("SELECT MAX(trade_date)::text AS td FROM ashare_risk_score")
            row = cur.fetchone()
            if not row or not row["td"]:
                return {"trade_date": None, "data": []}
            trade_date = row["td"]

        # risk score data for all watchlist-active stocks on that date
        cur.execute("""
            SELECT
                r.ts_code,
                s.name,
                w.strategy AS source_strategy,
                r.trade_allowed,
                r.block_reason,
                r.risk_score_total,
                r.position_cap_multiplier_final,
                r.detail_json
            FROM ashare_risk_score r
            JOIN ashare_watchlist w
                ON w.ts_code = r.ts_code
                AND w.lifecycle_status IN ('candidate','signaled')
            LEFT JOIN ashare_stock_basic s ON s.ts_code = r.ts_code
            WHERE r.trade_date = %s::date
            ORDER BY r.trade_allowed ASC, r.risk_score_total DESC
        """, (trade_date,))
        risk_rows = cur.fetchall()

        # rejected orders on that date for cross-reference
        cur.execute("""
            SELECT ts_code, reject_reason
            FROM ashare_sim_orders
            WHERE order_date = %s::date AND status = 'rejected'
        """, (trade_date,))
        reject_map = {}
        for rr in cur.fetchall():
            reject_map[rr["ts_code"]] = rr["reject_reason"]
        ...
        for r in risk_rows:
            ...
            d["cash_ok"] = reject_map.get(r["ts_code"], "").find("cash") == -1 if r["ts_code"] in reject_map else True
            d["limit_up_down_block"] = "limit" in (reject_map.get(r["ts_code"], "").lower())
            d["min_liquidity_ok"] = r["trade_allowed"] or "流动性" not in (r.get("block_reason") or "")
            d["sim_reject_reason"] = reject_map.get(r["ts_code"])

            out.append(d)

        return {"trade_date": trade_date, "data": out}
    finally:
        conn.close()
...
class ConceptMemberAdd(BaseModel):
    ts_codes: List[str]
...
def get_concept_list():
    """所有概念板块列表 + 成分股数量 + 当日整体涨跌"""
    conn = get_db()
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""
            WITH member_count AS (
                SELECT concept_id, COUNT(*) AS stock_count
                FROM ashare_concept_member
                GROUP BY concept_id
            ),
            latest_date AS (
                SELECT MAX(trade_date) AS td FROM ashare_daily_price
            ),
            concept_perf AS (
                SELECT cm.concept_id,
                       ROUND(AVG(
                           CASE WHEN dp2.close IS NOT NULL AND dp2.close != 0
                                THEN (dp.close - dp2.close) / dp2.close * 100
                           END
                       )::numeric, 2) AS avg_pct_chg
                FROM ashare_concept_member cm
                JOIN ashare_daily_price dp ON dp.ts_code = cm.ts_code
                CROSS JOIN latest_date ld
                LEFT JOIN LATERAL (
                    SELECT close FROM ashare_daily_price
                    WHERE ts_code = dp.ts_code AND trade_date < dp.trade_date
                    ORDER BY trade_date DESC LIMIT 1
                ) dp2 ON TRUE
                WHERE dp.trade_date = ld.td
                GROUP BY cm.concept_id
            )
            SELECT c.concept_id, c.concept_name, c.description, c.is_active,
                   COALESCE(mc.stock_count, 0) AS stock_count,
                   cp.avg_pct_chg,
                   c.created_at, c.updated_at
            FROM ashare_concept c
            LEFT JOIN member_count mc ON mc.concept_id = c.concept_id
            LEFT JOIN concept_perf cp ON cp.concept_id = c.concept_id
...
@app.get("/api/concept/{concept_id}/members")
def get_concept_members(concept_id: int):
    """某概念的成分股 + 当日涨跌"""
    conn = get_db()
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""
            ...
            SELECT cm.ts_code, s.name, cm.is_primary, cm.notes,
                   dp.close,
                   CASE WHEN dp2.close IS NOT NULL AND dp2.close != 0
                        THEN ROUND(((dp.close - dp2.close) / dp2.close * 100)::numeric, 2)
                   END AS pct_chg,
                   dp.amount,
                   cm.added_at
            FROM ashare_concept_member cm
            LEFT JOIN ashare_stock_basic s ON s.ts_code = cm.ts_code
            LEFT JOIN latest_date ld ON TRUE
            LEFT JOIN ashare_daily_price dp ON dp.ts_code = cm.ts_code AND dp.trade_date = ld.td
            LEFT JOIN LATERAL (
                SELECT close FROM ashare_daily_price
                WHERE ts_code = cm.ts_code AND trade_date < ld.td
                ORDER BY trade_date DESC LIMIT 1
            ) dp2 ON TRUE
            WHERE cm.concept_id = %s
            ORDER BY cm.is_primary DESC, pct_chg DESC NULLS LAST
        """, (concept_id,))
        rows = cur.fetchall()
...
@app.get("/api/concept/{concept_id}/performance")
def get_concept_performance(concept_id: int, days: int = 20):
    """某概念板块最近N日等权平均涨跌序列"""
    conn = get_db()
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""
            WITH members AS (
                SELECT ts_code FROM ashare_concept_member WHERE concept_id = %s
            ),
            recent_dates AS (
                SELECT DISTINCT trade_date FROM ashare_daily_price
                ORDER BY trade_date DESC LIMIT %s
            )
            SELECT dp.trade_date::text AS trade_date,
                   ROUND(AVG(dp.pct_chg)::numeric, 2) AS avg_pct_chg,
                   COUNT(*) AS stock_count
            FROM ashare_daily_price dp
            JOIN members m ON m.ts_code = dp.ts_code
            JOIN recent_dates rd ON rd.trade_date = dp.trade_date
        ...
        cur.execute("SELECT concept_name FROM ashare_concept WHERE concept_id = %s", (concept_id,))
        cname = cur.fetchone()
        return {"concept_name": cname["concept_name"] if cname else None, "days": days, "data": out}
    finally:
        conn.close()
...
def create_concept(body: ConceptCreate):
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""
            INSERT INTO ashare_concept (concept_name, description)
            VALUES (%s, %s)
        ...
        """, (body.concept_name, body.description))
        row = cur.fetchone()
        conn.commit()
        return {"message": "ok", "data": dict(row)}
    finally:
        conn.close()


@app.post("/api/concept/{concept_id}/members")
def add_concept_members(concept_id: int, body: ConceptMemberAdd):
    """批量添加成分股到概念板块"""
    conn = get_db()
    try:
        cur = conn.cursor()
        ...
        for ts in body.ts_codes:
            ...
            if body.is_primary:
                cur.execute("""
                    UPDATE ashare_concept_member SET is_primary = FALSE
                    WHERE ts_code = %s AND is_primary = TRUE
                """, (ts,))
                primary_set += cur.rowcount
            cur.execute("""
                INSERT INTO ashare_concept_member (concept_id, ts_code, is_primary, notes)
                VALUES (%s, %s, %s, %s)
                ON CONFLICT (concept_id, ts_code) DO UPDATE SET
                    is_primary = EXCLUDED.is_primary,
                    notes = COALESCE(EXCLUDED.notes, ashare_concept_member.notes)
            """, (concept_id, ts, body.is_primary, body.notes))
            inserted += 1
        conn.commit()
        return {"message": "ok", "inserted": inserted, "primary_cleared": primary_set}
    finally:
        conn.close()


@app.get("/api/concept/stock/{ts_code}")
def get_stock_concepts(ts_code: str):
    """查某只股票的主概念 + 所属全部概念"""
    conn = get_db()
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""
            SELECT c.concept_id, c.concept_name, cm.is_primary, cm.notes, cm.added_at
            FROM ashare_concept_member cm
            JOIN ashare_concept c ON c.concept_id = cm.concept_id AND c.is_active = TRUE
            WHERE cm.ts_code = %s
            ORDER BY cm.is_primary DESC, c.concept_name
        """, (ts_code,))
        rows = cur.fetchall()
        primary = None
        all_concepts = []
        for r in rows:
            d = {"concept_id": r["concept_id"], "concept_name": r["concept_name"],
                 "is_primary": r["is_primary"], "notes": r["notes"],
                 "added_at": r["added_at"].isoformat() if r["added_at"] else None}
            all_concepts.append(d)
            if r["is_primary"]:
                primary = r["concept_name"]
        return {"ts_code": ts_code, "primary_concept": primary, "concepts": all_concepts}
    finally:
        conn.close()
